...
select count(consultas.codm), medicos.nome from medicos left join consultas on medicos.codm = consultas.codm group by medicos.codm; -- Q1 Mostre a quantidade consultas atendidas por médico
select medicos.nome, pacientes.nome, consultas.hora from medicos inner join consultas on medicos.codm = consultas.codm inner join pacientes on consultas.codp = pacientes.codp where consultas.hora between '07:00' and '12:00' -- Q2 Mostre o nome dos medicos e pacientes em consulta realizadas pela manhã
select medicos.nome, pacientes.* from medicos left join consultas on medicos.codm = consultas.codm left join pacientes on consultas.codp = pacientes.codp; -- Q3 Mostre o nome dos médicos e para aqueles que tiverem consultas, os dados do paciente
select count(codm) from medicos group by nroa; -- Q4 Mostre o total de  medicos por ambulatório
select medicos.nome, medicos.especialidade, pacientes.doenca from medicos left join consultas on medicos.codm = consultas.codm left join pacientes on consultas.codp = pacientes.codp; -- Q5 Mostre a especialidade do médico,  a doenca do paciente para cada atendimento em consultas
select avg(pacientes.idade), medicos.nroa from consultas inner join pacientes on consultas.codp = pacientes.codp inner join medicos on consultas.codm = medicos.codm group by nroa; -- Q6 mostre a média de idade dos pacientes atendidos pelos médicos, por ambulatório
select min(consultas.codp), medicos.nome as Nome_Medico from medicos left join consultas on medicos.codm = consultas.codm left join pacientes on  consultas.codp = pacientes.codp group by medicos.nome order by min(consultas.codp); -- Q7 Mostre o nome do médico com o menor número de atendimentos em consultas.
select count(codm) from medicos group by especialidade; -- Q8 Mostre  a quantidade de médicos por especialidade
select medicos.nome from medicos left join consultas on medicos.codm = consultas.codm where consultas.codm is null; -- Q9 Mostre os dados do médico que não atendeu qualquer consulta
select sum(pacientes.idade) as SOMA_IDADES, medicos.nome from pacientes inner join consultas on pacientes.codp = consultas.codp inner join medicos on consultas.codm = medicos.codm group by medicos.codm order by SOMA_IDADES desc;-- Q10 mostre a soma das idades dos pacientes atendidos por médico, do maior para o menor